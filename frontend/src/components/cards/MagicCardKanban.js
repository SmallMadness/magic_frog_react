...
function MagicCardKanban({ card, onAddToDeck }) {
  ...
  // Prüfe, ob die Mana-Kosten zu lang sind (mehr als 3 Symbole)
  const hasLongManaCost = () => {
    if (!normalizedCard.mana_cost) return false;
    // Zähle die Anzahl der Mana-Symbole (jedes Symbol ist in {} eingeschlossen)
    const matches = normalizedCard.mana_cost.match(/{[^}]+}/g);
      ...
      <div className="card-inner">
        ...
        <div className="card-front">
          ...
          <div className="card-header">
            <div className={`card-title-container ${!normalizedCard.mana_cost || hasLongManaCost() ? 'full-width' : ''}`}>
              <h3 className="card-title" title={normalizedCard.name}>{normalizedCard.name}</h3>
            </div>
            {normalizedCard.mana_cost && !hasLongManaCost() && (
        ...
        <div className="card-back">
          ...
          <div className="card-body card-details">
            {hasLongManaCost() && normalizedCard.mana_cost && (
              <div className="detail-row">
                <span className="detail-label">Mana-Kosten:</span>
            ...
            {/* Details-Link wurde in die Fußzeile verschoben */}
          </div>
          <div className="card-footer">
            <Link to={`/card/${normalizedCard.id}`} className="detail-link" onClick={(e) => e.stopPropagation()}>
              Details anzeigen
            </Link>
            <button className="btn-add" onClick={handleAddToDeck} title="Zum Deck hinzufügen">+</button>
          </div>
